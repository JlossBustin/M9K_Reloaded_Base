...
concommand.Add("m9kr_belt_test", function(ply, cmd, args)
	local bg = tonumber(args[1]) or 0
	M9KR.BeltFed.Disabled = true
	local vm = ply:GetViewModel()
	if IsValid(vm) then
		vm:SetBodygroup(1, bg)
		print("[M9KR Belt Test] Set bodygroup 1 to " .. bg)
	end
end)

concommand.Add("m9kr_belt_enable", function()
	M9KR.BeltFed.Disabled = false
	print("[M9KR Belt] Re-enabled")
end)

-- Debug command to manually reset ALL bone and bodygroup manipulations on viewmodel
concommand.Add("m9kr_reset_bones", function()
	local ply = LocalPlayer()
	if not IsValid(ply) then return end

	local vm = ply:GetViewModel()
	if not IsValid(vm) then
		print("[M9KR] No valid viewmodel")
		return
	end

	local boneCount = vm:GetBoneCount() or 0
	print("[M9KR] Resetting " .. boneCount .. " bones and bodygroups on viewmodel: " .. tostring(vm:GetModel()))
	...
	-- Reset all bodygroups (fixes Ameli -> MG36 issue)
	for bgIndex = 0, 10 do
	...
	M9KR.BeltFed.LastWeaponClass = nil
	M9KR.BeltFed.LastViewModelModel = nil
	M9KR.BeltFed.NeedsBoneReset = false

	print("[M9KR] All bone and bodygroup manipulations reset!")
end)

-- Debug command to show current bone states
concommand.Add("m9kr_debug_bones", function()
	local ply = LocalPlayer()
	if not IsValid(ply) then return end
	...
	print("[M9KR] Debug info:")
	print("  Weapon: " .. (IsValid(weapon) and weapon:GetClass() or "none"))
	print("  VM Model: " .. tostring(vm:GetModel()))
	print("  VM Bone Count: " .. (vm:GetBoneCount() or 0))
	print("  LastWeaponClass: " .. tostring(M9KR.BeltFed.LastWeaponClass))
	print("  LastVMModel: " .. tostring(M9KR.BeltFed.LastViewModelModel))
	print("  NeedsBoneReset: " .. tostring(M9KR.BeltFed.NeedsBoneReset))
	print("  ManipulatedBones: " .. table.Count(M9KR.BeltFed.ManipulatedBones))

	-- Check for any bones with non-default scale
	local boneCount = vm:GetBoneCount() or 0
	local scaledBones = {}
	for boneID = 0, boneCount - 1 do
		local boneName = vm:GetBoneName(boneID)
		-- We can't easily read current manipulation, but we can list bones
		if boneName then
			table.insert(scaledBones, boneID .. "=" .. boneName)
		end
	end
	print("  Bones: " .. table.concat(scaledBones, ", "))
end)
