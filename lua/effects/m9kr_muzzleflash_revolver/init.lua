--[[
	M9K Reloaded - Revolver Muzzle Flash Effect
	Uses TFA Realistic Muzzleflashes 2.0 PCF particle system for optimal performance

	TFA Realistic 2.0 Properties:
	- Particle: muzzleflash_pistol_rbull / muzzleflash_pistol_rbull_optimized
	- Life: 0.1 seconds (longer for high-power revolvers)
	- HeatSize: 0.85 (large heatwave for powerful revolvers)
...
EFFECT.Life = 0.1
EFFECT.HeatSize = 0.85
EFFECT.FlashSize = 0.85
...
function EFFECT:Init(data)
	...
	if usingFallbackPos or correctedThirdPerson then
		if smokeEnabled then
			ParticleEffect("muzzleflash_pistol_rbull", self.vOffset, self.Dir:Angle())
		else
			-- Skip server-broadcast flash for local player when deferred system will handle it
			if IsValid(self.WeaponEntOG) and self.WeaponEntOG.m9kr_PendingMuzzleFlash then return end
			ParticleEffect("muzzleflash_pistol_rbull_optimized", self.vOffset, self.Dir:Angle())
		end
	else
		if smokeEnabled then
			ParticleEffectAttach("muzzleflash_pistol_rbull", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		else
			-- Skip server-broadcast flash for local player when deferred system will handle it
			if IsValid(self.WeaponEntOG) and self.WeaponEntOG.m9kr_PendingMuzzleFlash then return end
			ParticleEffectAttach("muzzleflash_pistol_rbull_optimized", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		end
	end
