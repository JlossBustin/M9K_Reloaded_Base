--[[
	M9K Reloaded - Silenced/Suppressed Muzzle Flash Effect
	Uses TFA Realistic Muzzleflashes 2.0 PCF particle system for optimal performance

	TFA Realistic 2.0 Properties:
	- Particle: muzzleflash_suppressed / muzzleflash_suppressed_optimized
	- Life: 0.085 seconds (short flash duration)
	- HeatSize: 0.65 (SMALLEST - suppressed weapons)
	- Dynamic light: 128 size (very dim), brightness 0.85 or disabled
...
function EFFECT:Init(data)
	...
	-- If we computed a corrected third-person position, use that directly
	-- (GetTracerShootPos re-fetches from bonemerge, overriding our correction)
	...
	-- smokeEnabled = 1: Use regular particles (with smoke baked in)
	-- smokeEnabled = 0: Use optimized particles (reduced smoke)
	if usingFallbackPos or correctedThirdPerson then
		-- Unattached particle at computed world position
		-- For scoped: fallback position below crosshair
		-- For corrected third person: offset-corrected muzzle position
		if smokeEnabled then
			ParticleEffect("muzzleflash_suppressed", self.vOffset, self.Dir:Angle())
		else
			ParticleEffect("muzzleflash_suppressed_optimized", self.vOffset, self.Dir:Angle())
		end
	else
		-- Attach to weapon for smooth tracking (first person, no offset correction needed)
		if smokeEnabled then
			ParticleEffectAttach("muzzleflash_suppressed", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		else
			ParticleEffectAttach("muzzleflash_suppressed_optimized", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		end
	end

	-- scotchmuzzleflash sprites (minimal for suppressed)
	...
	-- Dynamic lighting (very dim for suppressed, often disabled)
	...
	-- Heatwave effect (minimal for suppressed)
