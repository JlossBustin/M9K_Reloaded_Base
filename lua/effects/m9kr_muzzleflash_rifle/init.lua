--[[
	M9K Reloaded - Rifle Muzzle Flash Effect
	Uses TFA Realistic Muzzleflashes 2.0 PCF particle system for optimal performance

	TFA Realistic 2.0 Properties:
	- Particle: muzzleflash_6 / muzzleflash_6_optimized (based on cl_tfa_rms_optimized_smoke)
	- Life: 0.085 seconds
	- HeatSize: 0.80
	- Dynamic light: 512 size, orange/yellow color
...
function EFFECT:Init(data)
	...
	if usingFallbackPos or correctedThirdPerson then
		...
		if smokeEnabled then
			ParticleEffect("muzzleflash_6", self.vOffset, self.Dir:Angle())
		else
			ParticleEffect("muzzleflash_6_optimized", self.vOffset, self.Dir:Angle())
		end
	else
		-- Attach to weapon for smooth tracking
		if smokeEnabled then
			ParticleEffectAttach("muzzleflash_6", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		else
			ParticleEffectAttach("muzzleflash_6_optimized", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		end
	end
