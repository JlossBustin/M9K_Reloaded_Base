--[[
	M9K Reloaded - LMG (Light Machine Gun) Muzzle Flash Effect
	Uses TFA Realistic Muzzleflashes 2.0 PCF particle system for optimal performance

	TFA Realistic 2.0 Properties:
	- Particle: muzzleflash_minimi / muzzleflash_vollmer_optimized
	- Life: 0.1 seconds (longer for machine guns)
	- HeatSize: 0.80 (moderate heatwave for LMGs)
	- Dynamic light: 512 size
...
EFFECT.Life = 0.1
EFFECT.HeatSize = 0.80
EFFECT.FlashSize = 0.80
...
function EFFECT:Init(data)
	...
	if usingFallbackPos or correctedThirdPerson then
		if smokeEnabled then
			ParticleEffect("muzzleflash_minimi", self.vOffset, self.Dir:Angle())
		else
			ParticleEffect("muzzleflash_vollmer_optimized", self.vOffset, self.Dir:Angle())
		end
	else
		if smokeEnabled then
			ParticleEffectAttach("muzzleflash_minimi", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		else
			ParticleEffectAttach("muzzleflash_vollmer_optimized", PATTACH_POINT_FOLLOW, self.WeaponEnt, self.Attachment)
		end
	end
