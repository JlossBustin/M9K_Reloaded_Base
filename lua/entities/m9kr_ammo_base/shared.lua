--[[
	M9K Reloaded - Ammo Crate Base Entity

	All m9kr_ammo_* entities inherit from this base.
	Handles pickup, physics, 3D2D text rendering, and optional detonation.
	Detonation controlled by server ConVar m9kr_ammo_detonation.
]]--
...
function ENT:OnTakeDamage(dmginfo)
	...
	local cv_det = GetConVar("m9kr_ammo_detonation")
	local detonation_enabled = cv_det and cv_det:GetBool()
	...
	if IsValid(at) then
		...
		if (base and string.find(base, "m9kr_ammo_base")) or string.find(at:GetClass(), "m9kr_ammo_") then
			if not detonation_enabled then
...
function ENT:Think()
	...
	local cv_det = GetConVar("m9kr_ammo_detonation")
	if not cv_det or not cv_det:GetBool() then return true end
